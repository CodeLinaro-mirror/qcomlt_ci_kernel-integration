				 ...
				 void *data)
{
	...
	case LINK_UP:
		...
		break;
	case LINK_DOWN:
		/* Power down the MHI EP stack if link is up and link state is enabled */
		if (epf_mhi->link_enabled && epf_mhi->link_up) {
			mhi_ep_power_down(mhi_cntrl);
			epf_mhi->link_enabled = false;
		}
		epf_mhi->link_up = false;
		break;
	case BME:
		/* Power up the MHI EP stack if link is up and link state is not enabled */
		if (!epf_mhi->link_enabled && epf_mhi->link_up) {
			/* Power up the MHI EP controller */
			mhi_ep_power_up(mhi_cntrl);
			epf_mhi->link_enabled = true;
		}

		break;
	default:
		dev_err(&epf->dev, "Invalid MHI device notifier event: %ld\n", val);
		return NOTIFY_BAD;
	}
