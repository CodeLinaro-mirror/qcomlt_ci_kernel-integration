...
/*
 * PCI EPF driver for MHI Endpoint
 *
				 ...
				 void *data)
{
	...
	case CORE_INIT:
		...
		break;
	case LINK_UP:
		ret = mhi_ep_set_link_state(mhi_cntrl, LINK_STATE_UP);
		if (ret) {
			dev_err(dev, "Failed to set LINK UP state\n");
			return NOTIFY_BAD;
		}

		break;
	case LINK_DOWN:
		ret = mhi_ep_set_link_state(mhi_cntrl, LINK_STATE_DOWN);
		if (ret) {
			dev_err(dev, "Failed to set LINK DOWN state\n");
			return NOTIFY_BAD;
		}

		mhi_ep_power_down(mhi_cntrl);
		epf_mhi->enumerated = false;

		break;
	case BME:
		ret = mhi_ep_set_link_state(mhi_cntrl, LINK_STATE_BME);
		if (ret) {
			dev_err(dev, "Failed to set BME state\n");
			return NOTIFY_BAD;
		}

		if (!epf_mhi->enumerated) {
			/* Power up the MHI EP controller */
			mhi_ep_power_up(mhi_cntrl);
			epf_mhi->enumerated = true;
		}
	...
	case D_STATE:
		dstate = (*(int *)data);
		if (dstate == 0) {
			ret = mhi_ep_set_link_state(mhi_cntrl, LINK_STATE_D0);
			if (ret) {
				dev_err(dev, "Failed to set D0 state\n");
				return NOTIFY_BAD;
			}
		} else if (dstate == 3) {
			ret = mhi_ep_set_link_state(mhi_cntrl, LINK_STATE_D3);
			if (ret) {
				dev_err(dev, "Failed to set D3 state\n");
				return NOTIFY_BAD;
			}
		} else {
			dev_err(dev, "Invalid D state: %d\n", dstate);
			return NOTIFY_BAD;
		}
...
MODULE_DESCRIPTION("PCI EPF driver for MHI Endpoint");
