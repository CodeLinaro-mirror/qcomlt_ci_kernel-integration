			       ...
			       struct mtd_part_parser_data *data)
{
	...
	if (le32_to_cpu(ptable->numparts) > SMEM_FLASH_PTABLE_MAX_PARTS_V4) {
	...
	parts = kcalloc(le32_to_cpu(ptable->numparts), sizeof(*parts), GFP_KERNEL);
	if (!parts)
		return -ENOMEM;
	...
	pr_debug("SMEM partition table found: ver: %d len: %d\n",
		 le32_to_cpu(ptable->version), le32_to_cpu(ptable->numparts));
	*pparts = parts;
