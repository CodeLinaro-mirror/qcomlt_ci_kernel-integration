...
static netdev_tx_t mhi_ndo_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	spin_lock_irqsave(&mhi_ep_netdev->wrt_lock, flags);
	skb_queue_tail(&mhi_ep_netdev->tx_buffers, skb);
	spin_unlock_irqrestore(&mhi_ep_netdev->wrt_lock, flags);

	queue_work(mhi_ep_netdev->xmit_wq,
			&mhi_ep_netdev->xmit_work);
...
static int mhi_ep_net_probe(struct mhi_ep_device *mhi_dev,
