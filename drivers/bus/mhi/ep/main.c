		      ...
		      struct mhi_ep_ring_element *event)
{
	...
	ret = mhi_ep_ring_add_element(ring, event, 0);
	if (ret) {
		dev_err(dev, "Error adding element to event ring (%d)\n", event_ring);
		goto err_unlock;
	}

	/* Ensure that the ring pointer gets updated in host memory before triggering MSI */
	wmb();
			...
			enum mhi_ev_ccs code)
{
	...
	tmp = event.dword[0];
	tmp |= MHI_EP_ERE_TR_DWORD0(code, len);
	event.dword[0] = tmp;

	tmp = event.dword[1];
	tmp |= MHI_EP_ERE_TR_DWORD1(ring->ch_id, MHI_PKT_TYPE_TX_EVENT);
	event.dword[1] = tmp;
						...
						enum mhi_state state)
{
	...
	tmp = event.dword[0];
	tmp |= MHI_EP_ERE_SC_DWORD0(state);
	event.dword[0] = tmp;

	tmp = event.dword[1];
	tmp |= MHI_EP_ERE_SC_DWORD1(MHI_PKT_TYPE_STATE_CHANGE_EVENT);
	event.dword[1] = tmp;
...
int mhi_ep_send_ee_event(struct mhi_ep_cntrl *mhi_cntrl, enum mhi_ep_execenv exec_env)
{
	...
	tmp = event.dword[0];
	tmp |= MHI_EP_ERE_EE_DWORD0(exec_env);
	event.dword[0] = tmp;

	tmp = event.dword[1];
	tmp |= MHI_EP_ERE_SC_DWORD1(MHI_PKT_TYPE_EE_EVENT);
	event.dword[1] = tmp;
				...
				enum mhi_ev_ccs code)
{
	...
	tmp = event.dword[0];
	tmp |= MHI_EP_ERE_CC_DWORD0(code);
	event.dword[0] = tmp;

	tmp = event.dword[1];
	tmp |= MHI_EP_ERE_CC_DWORD1(MHI_PKT_TYPE_CMD_COMPLETION_EVENT);
	event.dword[1] = tmp;
...
void mhi_ep_suspend_channels(struct mhi_ep_cntrl *mhi_cntrl)
{
	...
	for (i = 0; i < mhi_cntrl->max_chan; i++) {
		...
		dev_dbg(dev, "Suspending channel (%d)\n", i);
...
void mhi_ep_resume_channels(struct mhi_ep_cntrl *mhi_cntrl)
{
	...
	for (i = 0; i < mhi_cntrl->max_chan; i++) {
		...
		dev_dbg(dev, "Resuming channel (%d)\n", i);
...
int mhi_ep_process_cmd_ring(struct mhi_ep_ring *ring, struct mhi_ep_ring_element *el)
{
	...
	ch_id = MHI_EP_CRE_GET_CHID(el);
	mhi_chan = &mhi_cntrl->mhi_chan[ch_id];
	ch_ring = &mhi_cntrl->mhi_chan[ch_id].ring;

	switch (MHI_EP_CRE_GET_TYPE(el)) {
...
int mhi_ep_process_tre_ring(struct mhi_ep_ring *ring, struct mhi_ep_ring_element *el)
{
	...
	if (ring->ch_id > mhi_cntrl->max_chan) {
		dev_err(dev, "Invalid channel ring (%d)\n", ring->ch_id);
		return -EINVAL;
	}
	 ...
	 */
	if (!mhi_chan->xfer_cb) {
		dev_err(dev, "No client available for channel (%d)\n", ring->ch_id);
		return -ENODEV;
	}
...
static int mhi_ep_cache_host_cfg(struct mhi_ep_cntrl *mhi_cntrl)
{
	...
	mhi_cntrl->ch_ctx_host_size = sizeof(struct mhi_chan_ctx) *
					mhi_cntrl->max_chan;
	mhi_cntrl->ev_ctx_host_size = sizeof(struct mhi_event_ctx) *
					mhi_cntrl->event_rings;
	mhi_cntrl->cmd_ctx_host_size = sizeof(struct mhi_cmd_ctx);
...
static void mhi_ep_queue_channel_db(struct mhi_ep_cntrl *mhi_cntrl,
				    u32 ch_int, u32 ch_idx)
{
	struct mhi_ep_ring *ring;

	for (; ch_int; ch_idx++, ch_int >>= 1) {
		if (ch_int & 1) {
			ring = &mhi_cntrl->mhi_chan[ch_idx].ring;
			 ...
			 */
			mhi_ep_mmio_disable_chdb_a7(mhi_cntrl, ch_idx);
...
static irqreturn_t mhi_ep_irq(int irq, void *data)
{
	...
	/* Check for cmd doorbell interrupt */
