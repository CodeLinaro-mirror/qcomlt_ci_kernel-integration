...
int mhi_ep_process_ring(struct mhi_ep_ring *ring)
{
	...
	if (ring->rd_offset != ring->wr_offset) {
		dev_err(dev, "Error processing the ring\n");
		return -EINVAL;
	}
...
int mhi_ep_ring_add_element(struct mhi_ep_ring *ring, struct mhi_ep_ring_element *element, int size)
{
	struct mhi_ep_cntrl *mhi_cntrl = ring->mhi_cntrl;
	struct device *dev = &mhi_cntrl->mhi_dev->dev;
	struct mhi_ep_ring_element *ring_shadow;
	...
	memcpy_toio(&ring_shadow[old_offset], element, sizeof(struct mhi_ep_ring_element));
