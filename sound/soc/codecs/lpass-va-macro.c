...
#define to_va_macro(_hw) container_of(_hw, struct va_macro_priv, hw)
...
static int va_clk_rsc_fs_gen_request(struct va_macro_priv *va, bool enable)
{
...
static int va_macro_mclk_enable(struct va_macro_priv *va, bool mclk_enable)
{
	struct regmap *regmap = va->regmap;
			       ...
			       struct snd_kcontrol *kcontrol, int event)
{
	...
	struct va_macro_priv *va = snd_soc_component_get_drvdata(comp);
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	u32 dec_id = mc->shift;
	struct va_macro_priv *va = snd_soc_component_get_drvdata(component);
...
{
	struct va_macro_priv *va = to_va_macro(hw);
	int val;
...
static int va_macro_register_fsgen_output(struct va_macro_priv *va)
{
	struct clk *parent = va->clk;
...
static int va_macro_probe(struct platform_device *pdev)
{
	...
	va = devm_kzalloc(dev, sizeof(struct va_macro_priv), GFP_KERNEL);
	if (!va)
		return -ENOMEM;

	va->dev = dev;
	va->hw_vote = devm_clk_get(dev, "macro");
	if (IS_ERR(va->hw_vote))
		return PTR_ERR(va->hw_vote);

	va->dcodec_vote = devm_clk_get(dev, "dcodec");
	if (IS_ERR(va->dcodec_vote))
		return PTR_ERR(va->dcodec_vote);

	va->clk = devm_clk_get(dev, "mclk");
	if (IS_ERR(va->clk))
		return PTR_ERR(va->clk);
	...
	clk_prepare_enable(va->hw_vote);
	clk_prepare_enable(va->dcodec_vote);
	clk_set_rate(va->clk, VA_MACRO_MCLK_FREQ);
	clk_prepare_enable(va->clk);

	res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	va->regmap = devm_regmap_init_mmio(dev, devm_ioremap_resource(dev, res),
					   &va_regmap_config);
	...
	return devm_snd_soc_register_component(dev, &va_macro_component_drv,
					       va_macro_dais,
					       ARRAY_SIZE(va_macro_dais));
err:
	clk_disable_unprepare(va->clk);
	clk_disable_unprepare(va->dcodec_vote);
	clk_disable_unprepare(va->hw_vote);
...
{
	struct va_macro_priv *va = dev_get_drvdata(&pdev->dev);
